...
|---|---|
| Home-page title, summary, pathway groups, or main cautions | `website/index.html` |
| Map types, thresholds, filename examples, or interpolation guidance | `website/pages/atlas-documentation/index.html` |
| Step-by-step commands and tutorial prose | `website/pages/tutorial/index.html` |
| NODDI, pyAFQ, whole-tract, or nodewise guidance | `website/pages/microstructure/index.html` |
| Manuscript settings, method distinctions, or interpretation language | `website/pages/methods/index.html` |
| Software, ROI sources, folders, troubleshooting, QA, or citation | `website/pages/resources/index.html` |
| Colors, spacing, typography, cards, code blocks, or mobile layout | `website/assets/css/styles.css` |
| Navigation toggle, copy buttons, or tutorial reading progress | `website/assets/js/site.js` |
| Runnable Inferior VTA–NAc example | `examples/mesoconnect_inferior_vta_nac_template.sh` |
| Published study and microstructure scripts | `analysis-code/` |
...
Copy one complete `<section class="tutorial-step">` block in `website/pages/tutorial/index.html`. Give it a unique ID such as `step-12`, update the visible number, and add a matching item in the sidebar navigation.

The important attributes are:

```html
<section id="step-12" class="tutorial-step" data-tutorial-step>
```

The sidebar link must match that ID:

```html
<a href="#step-12" data-step-link>...</a>
```
...
At the top of `website/assets/css/styles.css`, edit the values inside `:root`. Changing one token updates the entire site. For example:
...
1. Copy an existing page folder such as `website/pages/methods`.
2. Rename the folder and update the page `<title>`, meta description, breadcrumb, `h1`, and main content.
3. Add the new link to the header and footer on every existing page plus the new page.
4. Use root-relative paths that start with `/`, such as `/assets/css/styles.css`.
5. Run `npm run check`.

## Check changes locally

From the repository root, build and start the local server:
...
- The warning at the top of `website/pages/tutorial/index.html`.
- The commands in `website/pages/tutorial/index.html`.
- The comparison and reference tables in `website/pages/methods/index.html`.
